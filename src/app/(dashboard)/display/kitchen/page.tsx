...
import { getKitchenTickets, updateKitchenTicketStatus } from '@/lib/mockStorage';
...
export default function KitchenDisplayPage() {
  ...
  function loadTickets() {
    try {
      const mockTickets = getKitchenTickets();
      // Strictly filter to ensure only food items are shown
      const strictTickets = mockTickets
        .map(t => ({
          ...t,
          items: t.items.filter(item => item.type === 'food')
        }))
        .filter(t => t.items.length > 0); // Drop tickets that end up empty

      setTickets(strictTickets);
      setIsLoading(false);
    } catch {
      setError('Error loading kitchen tickets');
      setIsLoading(false);
    }
  }
  ...
  function handleUpdateStatus(ticketId: number, action: string) {
    const success = updateKitchenTicketStatus(ticketId, action);
    if (success) {
      loadTickets();
    }
  }
  ...
  function handleArchive(ticketId: number) {
    const success = updateKitchenTicketStatus(ticketId, 'archive');
    if (success) {
      loadTickets();
    }
  }
  ...
  if (isLoading) return <div className="p-8 text-kds-text font-semibold bg-kds-bg min-h-screen animate-pulse">Loading Kitchen Display...</div>;
  ...
  return (
    <div className="min-h-screen bg-kds-bg p-4 sm:p-6 page-enter">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center items-start gap-2 mb-6 border-b border-[#4A3A3C] pb-4">
        <div>
          <h1 className="font-display text-2xl sm:text-3xl text-kds-text font-bold tracking-wider">KITCHEN DISPLAY</h1>
          <p className="text-kds-text/40 text-xs font-body mt-0.5">Food orders from POS & Butchery</p>
        </div>
